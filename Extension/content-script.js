...
// ============================================================
// 🕵️‍♂️ [AutoBOT] 阶段 1: 立即注入嗅探器 (绕过 CSP)
// ============================================================
(function injectSniffer() {
  try {
    const script = document.createElement("script");
    // 使用 src 引入外部文件，绕过 CSP 限制
    script.src = chrome.runtime.getURL("scripts/sniffer.js");
    script.onload = function () {
      this.remove(); // 加载后移除标签，保持整洁
    };
    (document.head || document.documentElement).appendChild(script);
    console.log("💉 [AutoBOT] Sniffer script injected via src");
  } catch (e) {
    console.error("❌ [AutoBOT] Injection Failed:", e);
  }
})();

// ============================================================
// 🛠️ [AutoBOT] 阶段 2: 等待页面加载完成后再运行 UI 代码
// (修复 MutationObserver 报错)
// ============================================================
function initContentScript() {
  console.log("🚀 [AutoBOT] DOM Ready, initializing UI...");
  ...
  if (window.location.hostname === "wplace.live") {
    ...
    window.addEventListener("autobot-execute-script", async (event) => {
      ...
      console.log(
        `%c📡 Content script received execution request for: ${scriptName}`,
      ...
      try {
        ...
        if (response && response.success) {
          console.log(
            `%c✅ ${scriptName} executed successfully via content script`,
            "color: #39ff14; font-weight: bold;"
          );
        } else {
          console.error(
            `%c❌ Script execution failed:`,
            "color: #ff073a; font-weight: bold;",
            response?.error
          );
        }
      } catch (error) {
